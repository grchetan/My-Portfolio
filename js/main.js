...
if (dot && outline) {
  ...
  animateOutline();
  document
    .querySelectorAll('a, button, .card, .cert-card, .proj-card')
    .forEach((el) => {
...
// ══ SCROLL REVEAL ══
const reveals = document.querySelectorAll('.rv, .card, .cert-card, .proj-card');
if (reveals.length > 0) {
  ...
  contactForm.addEventListener('submit', async function (e) {
    ...
    const payload = {
      ...
      budget: document.getElementById('f-budget')
        ? document.getElementById('f-budget').value.trim()
        : '',
